...
void replace_gbitmap_color(GColor color_to_replace, GColor replace_with_color, GBitmap *im, BitmapLayer *bml){
	...
	#ifdef DEBUG_COLOR
	   printf("[MAIN][REPLACE]Palette has %d items", num_palette_items);
	#endif
	...
	for(int i = 0; i < num_palette_items; i++){
		...
		#ifdef DEBUG_COLOR
		printf("[MAIN][REPLACE]Palette[%d] = %s (alpha:%d)", i, get_gcolor_text(current_palette[i]),(current_palette[i].argb >>6));
		#endif
		...
		if ((color_to_replace.argb & 0x3F)==(current_palette[i].argb & 0x3F)){
			...
			#ifdef DEBUG_COLOR
			   printf("[MAIN][REPLACE][%d] replaced with %s (alpha:%d)", i, get_gcolor_text(current_palette[i]),(current_palette[i].argb >>6));
			#endif
