...
void LoadConfigFromStorage()
{
   ...
   {
     m_b_Weather_TemperatureInCelcius = persist_read_bool(KEY_TEMPERATURE_CELCIUS);
   }
   if (persist_exists(KEY_CLOCK_24H)) 
   {
     m_b_Time_Clock24h = persist_read_bool(KEY_CLOCK_24H);
   }
   if (persist_exists(KEY_DATE_STYLE)) 
   {
     m_i_Time_DateStyle = persist_read_int(KEY_DATE_STYLE);
   }
...
void SaveConfigToStorage()
{
   ...
   persist_write_bool(KEY_TEMPERATURE_CELCIUS, m_b_Weather_TemperatureInCelcius);
   persist_write_bool(KEY_CLOCK_24H, m_b_Time_Clock24h);
   persist_write_int(KEY_DATE_STYLE, m_i_Time_DateStyle);
...
void LoadDefaultConfig()
{
   ...
   m_i_Time_DateStyle = DATE_DD_MM_YY;
   m_b_Time_Clock24h = true;
